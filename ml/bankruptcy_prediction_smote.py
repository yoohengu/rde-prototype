"""
기업 부도 예측 머신러닝 모델 (SMOTE 적용 버전)
- 목표변수: 모형개발용Performance(향후1년내부도여부) 
- SMOTE 오버샘플링으로 클래스 불균형 해결
"""
...
print("=" * 80)
print("기업 부도 예측 머신러닝 모델 (SMOTE 적용)")
print("=" * 80)
...
# SMOTE 적용 (부도 기업 수에 따라 k_neighbors 자동 조정)
...
# 5. 특성 스케일링 (SMOTE 적용된 데이터로 스케일링)
print("\n[5] 특성 스케일링...")
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train_resampled)  # ← resampled 데이터 사용
...
for name, model in models.items():
    print(f"\n{name} 학습 중...")
    model.fit(X_train_scaled, y_train_resampled)  # ← resampled 데이터로 학습
...
summary_df.to_csv('model_comparison_results_smote.csv', index=False, encoding='utf-8-sig')  # ← 파일명 변경
print("✓ 모델 비교 결과 저장: model_comparison_results_smote.csv")

# 특성 중요도 저장 (Random Forest 또는 Gradient Boosting인 경우)
if best_model_name in ['Random Forest', 'Gradient Boosting']:
    feature_importance.to_csv('feature_importance_results_smote.csv',  # ← 파일명 변경
                             index=False, encoding='utf-8-sig')
    print("✓ 특성 중요도 저장: feature_importance_results_smote.csv")

# 11. 최종 요약
print("\n" + "="*80)
print("분석 완료! (SMOTE 적용)")
print("="*80)
print(f"\n✓ 데이터 샘플 수: {len(df)}")
print(f"✓ 사용된 특성 수: {X.shape[1]}")
print(f"✓ 부도 기업 비율: {y.mean():.2%}")
print(f"\n✓ SMOTE 적용 전 학습 데이터: {len(y_train)} (부도: {(y_train==1).sum()})")
print(f"✓ SMOTE 적용 후 학습 데이터: {len(y_train_resampled)} (부도: {(y_train_resampled==1).sum()})")
print(f"\n✓ 최고 성능 모델: {best_model_name}")
print(f"✓ 테스트 세트 ROC AUC: {best_result['roc_auc']:.4f}")
print(f"✓ 테스트 세트 F1 Score: {best_result['f1']:.4f}")

print("\n생성된 파일:")
print("  - model_comparison_results_smote.csv (모델 비교 결과)")
if best_model_name in ['Random Forest', 'Gradient Boosting']:
    print("  - feature_importance_results_smote.csv (특성 중요도)")
